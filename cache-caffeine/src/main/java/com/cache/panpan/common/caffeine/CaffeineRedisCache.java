...
import com.cache.panpan.base.notice.NoticeMessage;
...
public class CaffeineRedisCache extends RedisCache {
    ...
    public CaffeineRedisCache(String name, byte[] prefix,RedisTemplate<Object, Object> redisTemplate, CaffeineCache caffeineCache, long expiration, IClearNotice notice) {
    ...
    public void evict(Object key) {
        try {
            ...
            if(notice!=null){
                //先通知其他local缓存删除
                notice.send(new NoticeMessage(key, getName()));
            }
            //在删除本地缓存
    ...
    public void evict(RedisCacheElement element) {
        try {
            ...
            //先通知其他local缓存删除
            if(notice!=null){
                notice.send(new NoticeMessage(element.getKey(), getName()));
            }
            //在删除本地缓存
    ...
    public IClearNotice getMessageSend() {
        return notice;
    }

    public void setMessageSend(IClearNotice messageSend) {
